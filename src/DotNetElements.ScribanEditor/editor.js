    ...
    buildDecorations(view) {
        ...
        for (const { regexp, decoration, captureGroup, insideBracketsOnly } of scribanFunctionDecorators) {
            ...
            while ((match = re.exec(text)) !== null) {
                ...
                const inBrackets = lastOpenBracket > lastCloseBracket;

                const shouldHighlight = !insideBracketsOnly || inBrackets;
    ...
    for (const loop of loops) {
        if (pathParts[0] === loop.variable) {
            if (pathParts.length === 1) {
                // Show properties of loop items (e.g., "item." → show Value, Label)
                const loopVarDef = findVariableByPath(variableDefinitions, loop.arrayPath);
                if (loopVarDef && loopVarDef.childVars && loopVarDef.childVars.length > 0) {
                    // Get the first child as representative (all items have same structure)
                    const firstChild = loopVarDef.childVars[0];

                    // Show properties of the loop item
                    if (firstChild.properties) {
                        for (const prop of firstChild.properties) {
                            suggestions.push({
                                label: prop,
                                type: 'property',
                    ...
                    // Show child variables of the loop item
                    if (firstChild.childVars) {
                        for (const child of firstChild.childVars) {
                            suggestions.push({
                                label: child.name,
                                type: 'property',
                ...
                // Navigate deeper: "item.someProperty." → show nested properties
                const loopVarDef = findVariableByPath(variableDefinitions, loop.arrayPath);
                if (loopVarDef && loopVarDef.childVars && loopVarDef.childVars.length > 0) {
                    const firstChild = loopVarDef.childVars[0];
                    const remainingPath = pathParts.slice(1).join('.');
                    const targetVar = findVariableByPath([firstChild], remainingPath);
...
function getLoopContext(text, position, availableVariables) {
    // Scriban loop syntax: {{ for variable in collection }} or {{~ for variable in collection ~}}
    ...
    while ((match = forLoopRegex.exec(text)) !== null) {
        ...
        // Find corresponding end tag: {{ end }} or {{~ end ~}}
...
function scribanCompletions(element, context) {
    ...
    const inScribanBrackets = /\{\{~?[^}]*$/.test(textBefore);

    if (!inScribanBrackets) {
